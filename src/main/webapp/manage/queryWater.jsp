...
<body>
<div class="container">
    <h3 style="text-align: center">水电费详情页面</h3>
            ...
            <c:forEach items="${pageResult.items}" var="water" varStatus="w">
                <tr>
                    ...
                    <c:if test="${water.paystatus == '未缴费'}">
                        <td><button class="btn btn-default btn-sm" onclick="editWater(${water.id})">确认缴费</button>&nbsp;<button class="btn btn-default btn-sm" onclick="window.location.href='/man/queryWaterOne.do?id=${water.id}'">修改</button>&nbsp;<button class="btn btn-default btn-sm" onclick="delWater(${water.id})">删除</button></td>
                    </c:if>
                    <c:if test="${water.paystatus != '未缴费'}">
                        <td><button class="btn btn-default btn-sm" onclick="editWater(${water.id})">取消确认</button>&nbsp;<button class="btn btn-default btn-sm" onclick="window.location.href='/man/queryWaterOne.do?id=${water.id}'">修改</button>&nbsp;<button class="btn btn-default btn-sm" onclick="delWater(${water.id})">删除</button></td>
                    </c:if>
    ...
    <ul class="pagination">
        ...
        <li>
            ...
            <a href="${pageContext.request.contextPath}/man/queryWater.do?currentPage=${pageResult.currentPage-1}&room_ID=${map.room_ID[0]}" onclick="">&laquo;</a></li>
        ...
        <li>
            ...
            <a href="${pageContext.request.contextPath}/man/queryWater.do?currentPage=${pageResult.currentPage+1}&room_ID=${map.room_ID[0]}}">&raquo;</a></li>
